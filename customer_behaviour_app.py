...
def customer_prediction(gender_input,age_input,salary_input):
    try:
...
# Main
def main():
    st.title("Customer Behaviour Production App")
    ...
    if st.button("Predict Customer Purchase"):
        ...
        result,confidence = customer_prediction(gender_input,age_input,salary_input)
        ...
        if isinstance(result,str) and (result.startswith("Error")or result.startswith("Prediction Error")):
            ...
            st.success(f"Willl Customer Purchase? : {result_output}")
